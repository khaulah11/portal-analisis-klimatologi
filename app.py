...
st.caption("Pemprosesan, Quality Control, dan Analisis Lanjutan Siri Masa Hujan")
...
if not uploaded_files:
    st.info("Sila upload sekurang-kurangnya satu fail Excel untuk memulakan analisis.")
    st.stop()

# ============================================================
# ENJIN PENGECAMAN & PENGGABUNGAN DATA PINTAR
# ============================================================
...
def normalize_station_name(val):
    ...
    return val.upper() if val else "UNKNOWN_STATION"
...
def read_year_matrix_sheet(uploaded_file, year):
    ...
    # Unpivot kepada 4 lajur standard
...
def analyze_station_data(station_name, data_df):
    # Buang duplicate tarikh jika stesen diimport berulang kali
    data_df = data_df.drop_duplicates(subset=["Year", "Month", "Day"], keep="first").copy()
    
    # Bina daily matriks
    daily_dfs = []
    for yr in sorted(data_df["Year"].unique()):
...
# ============================================================
# PEMPROSESAN & PENGGABUNGAN DATA MENGIKUT STESEN (AUTO-GROUP)
# ============================================================

station_raw_store = {}

with st.spinner("⏳ Sedang memproses dan menggabungkan data stesen..."):
    for uploaded_file in uploaded_files:
        try:
            ...
            if len(year_sheets) > 0:
                st_name = normalize_station_name(os.path.splitext(uploaded_file.name)[0])
                if st_name not in station_raw_store:
                    station_raw_store[st_name] = []
                for yr in years:
                    df_yr = read_year_matrix_sheet(uploaded_file, yr)
                ...
                for s_name in sheet_names:
                    ...
                    st_name, df_clean = read_raw_station_sheet(raw_df, s_name)
...
# Analisis data yang telah digabungkan mengikut stesen unik
station_results = {}
for st_name, df_list in station_raw_store.items():
    if df_list:
        combined_df = pd.concat(df_list, ignore_index=True)
        station_results[st_name] = analyze_station_data(st_name, combined_df)
...
# ============================================================
# KAWALAN PAPARAN UTAMA & NAVIGASI BERPUSAT
# ============================================================
...
with col_nav2:
    st.write("")
    st.success(f"✅ **{len(station_results)} Stesen Unik** Dikesan")

current_res = station_results[selected_station]
current_target_year = current_res["target_year"]

# Metrik Pantas Stesen
kpi_c1, kpi_c2, kpi_c3, kpi_c4, kpi_c5 = st.columns(5)
kpi_c1.metric(f"Min {current_target_year}", f"{current_res['min_target_value']:.1f} mm" if current_res['min_target_value'] is not None else "N.A.", current_res['min_target_month'])
kpi_c2.metric(f"Maks {current_target_year}", f"{current_res['max_target_value']:.1f} mm" if current_res['max_target_value'] is not None else "N.A.", current_res['max_target_month'])
...
# ============================================================
# RUANG VISUALISASI BERSEPADU (SATU RUANG TAB SAHAJA)
# ============================================================
...
# ------------------------------------------------------------
with main_tabs[0]:
    st.subheader(f"📊 Analisis Siri Masa Lengkap — {selected_station}")
    ...
    with sub_plot_tabs[0]:
        ...
        ax.bar(x, current_res["rainfall_target"].values, width=0.60, color=st.session_state.bar_colors, edgecolor="black", linewidth=0.8, label=f"Total Rainfall {current_target_year}")
        ...
        ax.set_title(f"{selected_station}\nMonthly Rainfall {current_target_year} vs Mean Monthly Rainfall {YEAR_RANGE_TEXT}", fontsize=14, fontweight="bold")
        ax.set_xticks(x)
    ...
    with sub_plot_tabs[2]:
        ...
        ax.set_title(f"{selected_station}\nRainfall Anomaly {current_target_year} vs Mean ({YEAR_RANGE_TEXT})", fontsize=14, fontweight="bold")
        ax.set_xticks(x)
        ax.set_xticklabels(months)
    ...
    with sub_plot_tabs[3]:
        ...
        ax.set_title(f"{selected_station}\nMaximum Daily Rainfall - {current_target_year}", fontsize=14, fontweight="bold")
        ax.set_xticks(x)
        ax.set_xticklabels(months)
        ax.grid(True, axis="y", linestyle="--", alpha=0.4)
        plt.tight_layout()
    ...
    with sub_plot_tabs[4]:
        ...
        ax.set_title(f"{selected_station}\nNumber of Wet Days (≥0.1 mm) - {current_target_year}", fontsize=14, fontweight="bold")
        ax.set_xticks(x)
        ax.set_xticklabels(months)
        ax.grid(True, axis="y", linestyle="--", alpha=0.4)
        plt.tight_layout()
    ...
    with sub_plot_tabs[5]:
        ...
        for bar, val in zip(bars, current_res["std_daily"]):
            if pd.notna(val): ax.annotate(f"{val:.1f}", (bar.get_x() + bar.get_width()/2, val), xytext=(0, 6), textcoords="offset points", ha="center", fontsize=9, fontweight="bold")
        ax.set_title(f"{selected_station}\nDaily Rainfall Standard Deviation - {current_target_year}", fontsize=14, fontweight="bold")
        ax.set_xticks(x)
        ax.set_xticklabels(months)
        ax.grid(True, axis="y", linestyle="--", alpha=0.4)
        plt.tight_layout()
    ...
    with sub_plot_tabs[6]:
        if len(current_res["hist_values"]) > 0:
            ...
            ax.set_title(f"{selected_station}\nDistribution of Daily Rainfall - {current_target_year}", fontsize=14, fontweight="bold")
    ...
    with sub_plot_tabs[7]:
        if sum(current_res["category_values"]) > 0:
            ...
            ax.set_title(f"{selected_station}\nPercentage of Days by Category - {current_target_year}", fontsize=14, fontweight="bold")
            plt.tight_layout()
            st.pyplot(fig, use_container_width=True)
            plt.close(fig)
        else:
    ...
    with sub_plot_tabs[8]:
        target_data = current_res["all_daily"][current_res["all_daily"]["Year"] == current_target_year]
        b_data, b_labels = [], []
        for month in months:
            m_idx = months.index(month) + 1
            days_expected = calendar.monthrange(current_target_year, m_idx)[1] if current_target_year else 31
        ...
        if any(len(v) > 0 for v in b_data):
            ...
            ax.set_title(f"{selected_station}\nDaily Rainfall Distribution by Month - {current_target_year}", fontsize=14, fontweight="bold")
            ax.grid(True, axis="y", linestyle="--", alpha=0.4)
            plt.tight_layout()
            st.pyplot(fig, use_container_width=True)
            plt.close(fig)
...
with main_tabs[1]:
    ...
    if len(valid_means) > 0:
        ...
        with cp_left:
            fig_pub = px.bar(x=months, y=current_res["mean_monthly_total"].values, text=current_res["mean_monthly_total"].values, labels={'x':'Bulan', 'y':'Purata Hujan (mm)'}, title="Purata Penerimaan Hujan Bulanan")
